...
async function main() {
  const icon = await fs.readFile(path.join(brand, 'onedriver_icon.svg'), 'utf8');
  const round = await fs.readFile(path.join(brand, 'onedriver_icon_round.svg'), 'utf8');
  ...
  await fs.writeFile(path.join(brand, 'onedriver_icon_maskable.svg'), maskable);
  ...
  for (const size of [192, 512, 1024]) {
    await render(icon, path.join(brand, `onedriver_icon_${size}.png`), size);
  }
  await render(maskable, path.join(brand, 'onedriver_icon_maskable_512.png'), 512);
  await render(maskable, path.join(brand, 'onedriver_apple_touch_icon.png'), 180);
  // The native splash draws @drawable/ic_launcher_foreground, a vector. The
  // raster splash and app-icon copies that used to be written into drawable/
  // and drawable-nodpi were referenced by nothing and still shipped inside
  // every APK -- one of them 2.7 MB -- so they are no longer generated.
  await render(icon, path.join(mobile, 'assets/brand/onedriver_app_icon.png'), 512);
  for (const name of ['onedriver_wordmark', 'onedriver_wordmark_light', 'onedriver_lockup', 'onedriver_lockup_light', 'onedriver_play_feature']) {
    ...
    if (name.startsWith('onedriver_wordmark')) {
  ...
  const lockup = await fs.readFile(path.join(brand, 'onedriver_lockup.svg'), 'utf8');
  ...
  await render(preview, path.join(root, 'design-reference/onedriver-brand-2026-09-24.png'), 1400, 760);
  console.log('OneDriver: Android, Flutter, web/PWA, logo kit and review sheet exported.');
}
