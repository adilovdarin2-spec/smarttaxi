...
## Follow-up: dead region-picker wired up, no-phone self-review pass — `0064b3c`, `75e5865`

Note on infra first: status-doc history above got accidentally
truncated in this session by an earlier `Write` call that replaced the
whole file with just its own new section instead of appending — fixed
by restoring the full ~1900-line history from commit `6f3b794` and
re-appending everything written since. No content lost, just a scare.

Direct instruction: stop using the real device (a parallel session
owns it for driver-side work; rebuilding the APK now would confuse
that session's install), verify via Flutter web instead, self-review
the whole app, keep going until genuinely satisfied, then rebuild the
APK once at the very end.

- **Flutter web verification was blocked, root-caused, not just given
  up on**: the Browser pane's tab reports `document.hidden === true` /
  `visibilityState: "hidden"` to the page even when fronted via
  `tabs_select`, which makes Chromium throttle `requestAnimationFrame`
  indefinitely — confirmed step by step (WebGL works, CanvasKit
  `.wasm`/`.js` both fetch and instantiate fine, `window.flutterCanvasKit`
  exists, the `<flt-scene>` DOM node exists) but zero frames ever get
  painted (`<flt-scene>` stayed empty after 5+ minutes, `perfNow` at
  296000ms). Tried debug (DDC) and release builds, plain static server
  and the COI-headers server, tab fronting, and a `document.hidden`
  property override (doesn't affect the browser's real internal
  scheduling, cosmetic only) — none unstuck it. `Claude in Chrome` was
  not connected as a fallback. This is an environment limitation of
  this session's tooling, not a bug in the app — flagged directly
  instead of silently claiming visual verification that didn't happen.
- **Found and fixed a real, significant bug via code reading**:
  `_chooseRegion()` and `_confirmDetectedRegion()` in
  `passenger_shell.dart` were fully stubbed — `_chooseRegion` silently
  returned `_selectedRegion ?? _regions.first` instead of showing the
  already-built, fully-functional `_RegionSelectSheet` (search +  list),
  and `_confirmDetectedRegion` always auto-accepted the GPS-detected
  region instead of showing the already-built `_RegionConfirmSheet`
  ("Ваш регион: X? — Изменить / Да, верно"). `flutter analyze`'s
  "`_RegionConfirmSheet` isn't referenced" warning was the tell that
  led to this. With 12 active regions now configured
  (Атакент/Мырзакент/Жетысай/Киров/Асыката/Достык/Ынтымак/Бирлик/
  Фирдоуси/Жана Жол/Мактаарал/Атамекен, all `is_active: true` per
  `apps/api/src/db/migrations.js`), this meant passengers had **no way
  at all** to manually pick or correct their region — a real,
  previously-invisible defect, not a cosmetic one. Wired both
  functions to actually call `showModalBottomSheet`; dismissing
  without an explicit choice falls back to the same safe defaults the
  stubs used, so no regression for the "user just backs out" case.
- **Added a missing feature this bug pointed at**: there was no way to
  change region *later* either (only at first-launch geolocation), so
  added a "Регион" row to Settings → Аккаунт, same `_SettingsRow`
  pattern as the existing Язык/Тема rows, opening the same picker.
- Verified region-picker wiring by A/B testing: `git stash` on just
  `passenger_shell.dart`, re-ran the one already-failing widget test
  (`passenger home has map, order sheet, route, tariff, price, and
  action`, fails on `does not contain '_MapRoundButton'`) with the
  change removed — identical failure either way, confirming it's
  pre-existing and unrelated to this change, not something I broke.

**Verified:** `flutter analyze` clean (3 pre-existing warnings, down
from 4 — `_RegionConfirmSheet` unused warning resolved by actually
using it). `flutter test`: 25 passed / 5 failed both before and after
the region-picker change (stash-tested), so no regression introduced;
the 5 failures are pre-existing and were already failing identically
with the change reverted. **Could not verify live** — see the
Flutter-web root-cause writeup above; this round's region-picker fix
and settings addition are code-verified only (analyze + targeted A/B
test), not screenshot-verified, and are flagged to the user as such.
Not rebuilding the APK yet — continuing the self-review pass per
instruction, will rebuild once actually satisfied.
