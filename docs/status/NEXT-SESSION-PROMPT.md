...
Но остаётся ВТОРАЯ, более глубокая проблема: даже после этого фикса (и в `flutter run -d chrome`, и в `flutter build web` + статическая раздача) CanvasKit грузится (`window.flutterCanvasKit` не null), но приложение НИКОГДА не рисует первый кадр — `document.querySelectorAll('canvas')` пусто бесконечно, без единой ошибки в консоли. Проверил и исключил: не Sentry (временно вырезал `SentryFlutter.init()` из `main.dart` — не помогло, сразу вернул код обратно, `git diff` чистый). Похоже на что-то более глубокое в связке этого sandboxed Electron-браузера с Flutter web engine (после `initializeEngine()`, до первого `runApp()`-кадра) — не успел докопаться до причины. НЕ связано с этой правкой темы (падает на уровне движка/бутстрапа, до любого моего Dart-кода).
