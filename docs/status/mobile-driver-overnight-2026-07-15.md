...
**Two more one-click, no-undo admin actions from the same audit got a confirmation step**, in a follow-up pass: blocking a driver (suspends them from the whole service instantly, across every region) now opens a reason-required confirm panel and returns to the driver-detail view afterward rather than closing it; deactivating a region (cuts off every driver and client there from new orders at once) now opens a plain confirm panel explaining the blast radius. Unblocking a driver and activating a region stay one-click, since both are purely corrective/additive. Tariff-toggle and road-alert-hide have the same gap but were left alone this round — lower blast radius, and there's still a long tail of similar polish (FINANCE-role graceful degradation on 3 tabs, pagination past the 100-row fetch caps, a live driver map, broadcast notifications) the original audit found that didn't make tonight's cut; worth a dedicated pass later rather than trying to close all of it in one sitting.
