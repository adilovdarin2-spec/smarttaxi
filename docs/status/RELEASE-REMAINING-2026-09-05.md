...
|---|---|---|
| Passenger/driver visual parity | Continue the remaining driver/nested error/settings comparison against the current reference boards | The connected phone now covers native passenger home, address picker, route, driver-found, active trip, receipt and rating in the local lifecycle. Driver lifecycle evidence exists, but every current driver/nested error/settings state still needs its own parity comparison; shared styling alone is not full-state acceptance. |
...
| Native driver route layer | Reinspect the current driver navigator on the phone after the style-layer migration | Passenger physical QA now confirms the style-layer route below readable labels. Current driver code has the same GeoJSON/style-layer implementation and regression test, but the latest build's driver navigator still needs its own physical before/after capture before this row can close. |
